<template>
    <v-dialog v-model="dialog" persistent max-width="500px">
        <template v-slot:activator="{ on }">
            <v-btn color="primary" dark v-on="on">로그인(Log in)</v-btn>
        </template>
        <v-card>
            <v-card-title>
                ...
                <v-container grid-list-md>
                    <v-layout wrap>
                        ...
                        <v-flex xs12>
                            ...
                            v-model="userInfo.password" required></v-text-field>
                        </v-flex>
                    </v-layout>
                </v-container>
            </v-card-text>
    ...
    methods: {
        btnClick($event){
            ...
            if($event.target.innerHTML == "확인"){
                console.log("입력된 정보: 이메일: " + this.userInfo.email + " 패스워드: " + this.userInfo.password)
            }
            this.userInfo.email = ''
